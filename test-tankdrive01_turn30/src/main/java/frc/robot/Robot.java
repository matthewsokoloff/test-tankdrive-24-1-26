...
public class Robot extends LoggedRobot {
  ...
  public void teleopPeriodic() {
    ...
    // B turns it clockwise, X turns it counterclockwise
    if (m_controller.getBButtonPressed()) {
      m_drive.snapToAngle30(true);
    }

    if (m_controller.getXButtonPressed()) {
      m_drive.snapToAngle30(false);
    }

    // Y faces to 0 degrees, A faces to 180 degrees
    if (m_controller.getYButtonPressed()) {
      m_drive.turnToAbsoluteAngle(0);
    }
    
    if (m_controller.getAButtonPressed()) {
      m_drive.turnToAbsoluteAngle(180);
    }

    // if it's turning, continue the turn
    if (m_drive.isTurning()) {
      m_drive.updateTurn();
    }
    }
